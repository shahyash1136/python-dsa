...
class LinkedList:
    ...
    def _is_palindrome(self):
        if self.head is None:
            ...
            reverse_second_half = self._reverse_from(middle_node.next)

            start_list = self.head

            while start_list is not None and reverse_second_half is not None:
                if start_list.val == reverse_second_half.val:
